...
const Friends = (props) => {
    ...
    return (<div>
        {props.haveBeenLoaded ? <img src="https://pa1.narvii.com/6917/b2385a091b6aa61f0240c6d6f2016a4722845e1br1-320-320_hq.gif" /> : <span className={classes.numberOfPage}>Это самая {props.pageNumber}-я страничка</span>}<div className={classes.buttons}>
            {props.isActiveButton ? <button onClick={slideLeft}>Left</button> : <button className={classes.buttondis} onClick={slideLeft}>Left</button>}   {props.isActiveButton ? <button onClick={slideRight}>Right</button> : <button className={classes.buttondis} onClick={slideRight}>Right</button>} </div>
